...
5. Syncs the Steam client into the prefix: the `HKLM\Software\Wow6432Node\Valve\Steam` `InstallPath` registry value, plus `steamclient.dll` + `steamclient64.dll` + `GameOverlayRenderer64.dll` + `Steam.dll` + `steam.exe` copied from the installed Steam client's `legacycompat/` dir (found via the Steam-library resolver's roots; symlinked sources are dereferenced). The copy set and the `SteamService.exe -> steam.exe` rename replicate Proton's own prefix task, and like Proton the sync is change-tracked: the marker stores a signature of the source files (name/mtime/size), so a Steam client self-update re-copies on the next launch - a copy-once seed goes stale because Valve updates `legacycompat` and the native `steamclient.so` in place. Proton would do this itself given `STEAM_COMPAT_CLIENT_INSTALL_PATH`, but umu resets that variable to empty (it honors a pre-set `STEAM_COMPAT_INSTALL_PATH` but not the client path), so the launcher carries the sync. boiii-lineage clients (BOIII, t7x, CB Launcher's BO3) gate on the registry value + `steam.exe`, then their `steam_proxy` loads `steamclient64.dll` to build a Steam context - a stub alone passes the gate but hangs; a Steam-less host degrades with a clear message. CB Launcher's BO3 additionally gets `-nosteam` seeded via `launchOptions` by default, because BOIII's Steam integration is an optional sidecar and Valve's library fatally asserts ("stalled cross-thread pipe") when no running Steam client is reachable. The same setup step also disables Windows tray-icon forwarding per prefix (`Software\Wine\Explorer` `ShowSystray=N` + the `NoTrayItemsDisplay` policy, both read by this wine's explorer): forwarded tray icons strobe through Plasma's XWayland tray proxying at launch and stress it, and a sandboxed game session has no usable tray.
...
CB Launcher spawns every title through one `CreateProcessW`, so it drives a single prefix and one Proton. `cblauncher.subProton` overrides that per game: a background watcher (`subWatch`) polls for each listed exe, confirms it belongs to the CB prefix (reading `WINEPREFIX` from `/proc/<pid>/environ`, so it never re-routes the already-rerouted instance), kills it, and relaunches it detached in its own umu prefix under that exe's chosen `protonPath` - so one game can run a different Proton (e.g. Proton-CachyOS) while the rest of CB stays on the launcher default. Each subProton entry mirrors the standalone-client knobs: `protonPath`, `env`, `extraArgs`, `gameDir`/`extraGameDirs`, and `extraWinetricks` (added on top of the game's CB base verb set, exactly like the additive `extraArgs`).

`launchOptions`, `configFiles`, `gameSettings`, and the display-derived `maxFps` form the declarative settings surface: they seed per-exe CB launch arguments into `properties.json`, enforce per-game `seta` cvars into the games' config files, and set per-exe Wine AppDefaults and dxvk.conf options - all without hand-editing files inside the prefix. The same `cod_seta` upsert also enforces Plutonium's T5/T6 `raw_input "1"` into the prefix's Plutonium storage on every launch path that can run those titles (standalone, CB prefix, rerouted plutonium.exe), and the standalone Plutonium client carries the display-derived cap as `DXVK_FRAME_RATE`.
