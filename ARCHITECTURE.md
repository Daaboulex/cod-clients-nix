...
5. Seeds the Steam client into the prefix (marker-guarded, once): the `HKLM\Software\Wow6432Node\Valve\Steam` `InstallPath` registry value, plus `steamclient64.dll` + `GameOverlayRenderer64.dll` + `steam.exe` copied from the installed Steam client's `legacycompat/` dir (found via the Steam-library resolver's roots). boiii-lineage clients (BOIII, t7x, CB Launcher's BO3) gate on the registry value + `steam.exe`, then their `steam_proxy` loads `steamclient64.dll` to build a Steam context - a stub alone passes the gate but hangs. Proton copies from the same `legacycompat/` source but umu never tells it where Steam is, so it skips the copy; a Steam-less host degrades with a clear message.
